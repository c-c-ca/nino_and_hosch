...
class NimTUI {
     ...
     */
    public NimTUI () {
        this.user = new ScoredInteractivePlayer("Interactive Player");
        this.computer = new ScoredCleverPlayer("Clever Player");
        this.game = null;
    ...
    private void playGame (int numberOfSticks,
        ...
        if (userPlaysFirst)
            ...
            game = new Game (computer, user,
                numberOfSticks);
        new PlayerController((InteractivePlayer)user,game,in);
        while (!game.gameOver()) {
            game.play();
            reportPlay(game.previousPlayer());
        }
        recordScore(game.winner());
        reportWinner(game.winner());
        reportScore(user);
        reportScore(computer);
    }
    ...
    private void recordScore (Player player) {
        int score;
        if (player instanceof ScoredPlayer) {
            score = ((ScoredPlayer)player).score() + 1;
            ((ScoredPlayer)player).setScore(score);
        }
    }
    ...
    private void reportScore (Player player) {
        if (player instanceof ScoredPlayer) {
            System.out.println(
                player.name() + ": " + ((ScoredPlayer)player).score());
        }
    }
}
