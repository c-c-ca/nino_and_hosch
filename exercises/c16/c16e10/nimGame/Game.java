...
class Game {
    ...
    public void saveGame (String fileName) {
        try {
            ...
            out.writeInt(sticksLeft());
            out.close();
        } catch (IOException e) {
            System.out.println(
                "Unable to save game: " + e);
        }
    }

    public static Game restoreGame (String fileName) {
        try {
            DataInputStream in =
                new DataInputStream(
                    new FileInputStream(fileName));

            Player player1 = Player.restorePlayer(in);
            Player player2 = Player.restorePlayer(in);
            int sticks = in.readInt();
            in.close();
            Game game = new Game(player1,player2,sticks);
            game.previousPlayer = player2;
            return game;
        } catch (IOException e) {
            System.out.println(
                "Unable to restore game: " + e);
            return null;
        }
    }
}
