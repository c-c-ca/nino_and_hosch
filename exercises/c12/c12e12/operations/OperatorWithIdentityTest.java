...
public class OperatorWithIdentityTest {
    ...
    /**
     * Test the foldr method defined in OperatorUtilities.
     */
    @Test
    public void testFoldr () {
        // test an empty list
        List<Integer> list = makeList(0);
        assertEquals(0,ou.foldr(add,list));
        assertEquals(1,ou.foldr(mult,list));
        // test with 1 item
        list = makeList(1);
        assertEquals(1,ou.foldr(add,list));
        assertEquals(1,ou.foldr(mult,list));
        // test a typical list
        list = makeList(4);
        assertEquals(10,ou.foldr(add,list));
        assertEquals(24,ou.foldr(mult,list));
    }
}
