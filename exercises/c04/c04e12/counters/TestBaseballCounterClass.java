...
class BaseballCounterTest {
    ...
    private void testIncrementStrikes () {
        ...
        verify(counter.strikes() == 1, "increment strike count, 0 strikes");
        verify(counter.balls() == 1, "increment strike count, ball count");
        // strike 2
        counter.incrementStrikes();
        verify(counter.strikes() == 2, "increment strike count, 1 strike");
        verify(counter.balls() == 1, "increment strike count, ball count");
        // strike 3
        counter.incrementStrikes();
        verify(counter.strikes() == 0, "increment strike count, 2 strikes");
    ...
    private void testIncrementBalls () {
        ...
        verify(counter.balls() == 1, "increment ball count, 0 balls");
        // ball 2
        counter.incrementBalls();
        verify(counter.strikes() == 1, "increment ball count, strike count");
        verify(counter.balls() == 2, "increment ball count, 1 ball");
        // ball 3
        counter.incrementBalls();
        verify(counter.strikes() == 1, "increment ball count, strike count");
        verify(counter.balls() == 3, "increment ball count, 2 balls");
        // ball 4
        counter.incrementBalls();
        verify(counter.strikes() == 0, "increment ball count, strike count");
        verify(counter.balls() == 0, "increment ball count, 3 balls");
    }
